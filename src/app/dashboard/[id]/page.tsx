  ...
  if (!userData) {
    return (
      <div className="w-full min-h-screen flex flex-col justify-center items-center bg-neutral-950 text-neutral-400 p-4">
        <div className="text-red-400 mb-4">No active session found. Please log in again.</div>
        <Link href="/login" className="bg-amber-500 text-black px-4 py-2 rounded-xl text-sm font-bold">
          Go to Login
        </Link>
      </div>
    );
      ...
      <div className="w-full max-w-md border border-neutral-900 rounded-2xl flex flex-col p-8 bg-neutral-900/40 backdrop-blur-md shadow-2xl relative overflow-hidden">
        ...
        <div className="w-full flex flex-col gap-3 relative z-10 border-t border-neutral-900 pt-6">
          ...
          {!userData.isVerified && (
            ...
            >
              <span>{verifyingEmail ? "Sending Link..." : "Verify Email Address"}</span>
          ...
          >
            <span>{resettingPassword ? "Sending Reset..." : "Reset Password"}</span>
            <span className="text-xs opacity-60">→</span>
          </button>
        </div>
        ...
        <p className="mt-8 text-xs text-neutral-600 text-center relative z-10">
          Finished viewing?{" "}
          <Link href="/dashboard" className="text-neutral-400 hover:text-white transition-colors ml-1">
            Return to Dashboard
          </Link>
        </p>
